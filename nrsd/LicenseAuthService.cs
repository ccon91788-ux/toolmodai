...
public static class LicenseAuthService
{
    ...
    public static async Task<LoginResult> LoginAsync(string licenseKey)
    {
        try
        {
            ...
            if (licenseKey == "nrovoz")
            {
                return new LoginResult
                {
                    Success = true,
                    Token = "fake_token_for_nrovoz",
                    Message = "Thành công",
                    CustomerName = "Tool Server Ngọc Rồng Sao Đen",
                    LicenseExpiresAt = "31/12/2026"
                };
            }
    ...
    public static async Task<bool> SendHeartbeatAsync()
    {
        try
        {
            ...
            if (token == "fake_token_for_nrovoz") return true;
