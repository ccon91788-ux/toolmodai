...
public class GameScr : mScreen, IChatable
{
	...
	public static void loadCamera(bool fullmScreen, int cx, int cy)
	{
		...
		cmdBarW = gW;
		cmdBarX = 0;
		cmdBarY = GameCanvas.h - PaintG.hTab - cmdBarH;
		girlHPBarY = 0;
	...
	public bool isAttack()
	{
		...
		if (Char.myCharz().mobFocus != null)
		{
			...
			int num = -1;
			int num12 = Res.abs(Char.myCharz().cx - cmx);
			if (Char.myCharz().charFocus != null)
			{
				num = Res.abs(Char.myCharz().cx - Char.myCharz().charFocus.cx);
			}
			else if (Char.myCharz().mobFocus != null)
			{
				num = Res.abs(Char.myCharz().cx - Char.myCharz().mobFocus.x);
			}
			if ((Char.myCharz().mobFocus.status == 1 || Char.myCharz().mobFocus.status == 0 || Char.myCharz().myskill.template.type == 4 || num == -1 || num > num12) && Char.myCharz().myskill.template.type == 4)
			{
			...
			int num13 = Math.Abs(Char.myCharz().cx - Char.myCharz().mobFocus.getX());
			int num14 = Math.Abs(Char.myCharz().cy - Char.myCharz().mobFocus.getY());
			Char.myCharz().cvx = 0;
			if (num13 <= Char.myCharz().myskill.dx && num14 <= Char.myCharz().myskill.dy)
			{
				if (Char.myCharz().myskill.template.id == 20)
				{
					return true;
				}
				if (num14 > num13 && Res.abs(Char.myCharz().cy - Char.myCharz().mobFocus.getY()) > 30 && Char.myCharz().mobFocus.getTemplate().type == 4)
				{
				...
				{
					int num16 = ((Char.myCharz().cx > Char.myCharz().mobFocus.getX()) ? 1 : (-1));
					if ((TileMap.tileTypeAtPixel(Char.myCharz().mobFocus.getX() + num15 * num16, Char.myCharz().cy + 3) & 2) != 2)
					{
						flag2 = true;
					}
				}
				if (num13 <= num15 && !flag2)
				{
					if (Char.myCharz().cx > Char.myCharz().mobFocus.getX())
					{
						int num17 = Char.myCharz().mobFocus.getX() + num15 + (flag ? 30 : 0);
						...
						{
							Char.myCharz().cx = num17;
						}
						Char.myCharz().cdir = -1;
					}
					else
					{
						int num18 = Char.myCharz().mobFocus.getX() - num15 - (flag ? 30 : 0);
						int num19 = Char.myCharz().mobFocus.getX();
						bool flag4 = false;
						while (num19 > num18)
						{
							if (TileMap.tileTypeAtPixel(num19, Char.myCharz().cy + 3) == 8 || TileMap.tileTypeAtPixel(num19, Char.myCharz().cy + 3) == 4)
							{
								flag4 = true;
								break;
							}
						...
						if (flag4)
						{
							Char.myCharz().cx = num19 + 24;
						}
						else
						{
							Char.myCharz().cx = num18;
						}
						Char.myCharz().cdir = 1;
					}
					Service.gI().charMove();
			...
			int num2 = (Char.myCharz().myskill.dx - ((!flag5) ? 20 : 50)) * ((Char.myCharz().cx > Char.myCharz().mobFocus.getX()) ? 1 : (-1));
			if (num13 <= Char.myCharz().myskill.dx)
			{
				num2 = 0;
			}
			Char.myCharz().currentMovePoint = new MovePoint(Char.myCharz().mobFocus.getX() + num2, Char.myCharz().mobFocus.getY());
			Char.myCharz().endMovePointCommand = new Command(null, null, 8002, null);
			GameCanvas.clearKeyHold();
			GameCanvas.clearKeyPressed();
			return false;
		...
		if (Char.myCharz().npcFocus != null)
		{
			...
			int num3 = Math.Abs(Char.myCharz().cx - Char.myCharz().npcFocus.cx);
			int num4 = Math.Abs(Char.myCharz().cy - Char.myCharz().npcFocus.cy);
			if (num4 > 40)
			{
			...
			{
				int num5 = (20 + Res.r.nextInt(20)) * ((Char.myCharz().cx > Char.myCharz().npcFocus.cx) ? 1 : (-1));
				Char.myCharz().currentMovePoint = new MovePoint(Char.myCharz().npcFocus.cx + num5, Char.myCharz().cy);
				Char.myCharz().endMovePointCommand = new Command(null, null, 8002, null);
				GameCanvas.clearKeyHold();
				GameCanvas.clearKeyPressed();
			}
		...
		if (Char.myCharz().charFocus != null)
		{
			...
			int num6 = Math.Abs(Char.myCharz().cx - Char.myCharz().charFocus.cx);
			int num7 = Math.Abs(Char.myCharz().cy - Char.myCharz().charFocus.cy);
			if (Char.myCharz().isMeCanAttackOtherPlayer(Char.myCharz().charFocus) || Char.myCharz().isSelectingSkillBuffToPlayer())
			{
				...
				Char.myCharz().cvx = 0;
				if (num6 <= Char.myCharz().myskill.dx && num7 <= Char.myCharz().myskill.dy)
				{
					if (Char.myCharz().myskill.template.id == 20)
					{
						return true;
					...
					if ((TileMap.tileTypeAtPixel(Char.myCharz().cx, Char.myCharz().cy + 3) & 2) == 2)
					{
						int num9 = ((Char.myCharz().cx > Char.myCharz().charFocus.cx) ? 1 : (-1));
						if ((TileMap.tileTypeAtPixel(Char.myCharz().charFocus.cx + num8 * num9, Char.myCharz().cy + 3) & 2) != 2)
						{
							flag6 = true;
						}
					}
					if (num6 <= num8 && !flag6)
					{
						if (Char.myCharz().cx > Char.myCharz().charFocus.cx)
						{
							Char.myCharz().cx = Char.myCharz().charFocus.cx + num8;
							Char.myCharz().cdir = -1;
						}
						else
						{
				...
				int num10 = (Char.myCharz().myskill.dx - 20) * ((Char.myCharz().cx > Char.myCharz().charFocus.cx) ? 1 : (-1));
				if (num6 <= Char.myCharz().myskill.dx)
				{
					num10 = 0;
				}
				Char.myCharz().currentMovePoint = new MovePoint(Char.myCharz().charFocus.cx + num10, Char.myCharz().charFocus.cy);
				Char.myCharz().endMovePointCommand = new Command(null, null, 8002, null);
				GameCanvas.clearKeyHold();
				GameCanvas.clearKeyPressed();
				return false;
	...
	{
		if (gamePad.disableClickMove() || NRO_v247.Mods.ModBootstrap.IsAutoMoveActive())
		{
			return;
		}
		Char.myCharz().cancelAttack();
	...
	{
		// Flush queue đổi map toàn cục để waypoint/manual không phụ thuộc vòng lặp xmap.
		NRO_v247.Mods.Xmap.XmapNavigator.FlushPendingMapChange();
		NRO_v247.Mods.Xmap.XmapNavigator.UpdateMapChangeWatchdog();
...
//		SmallImage.update();
		try
		{
			...
			for (int i9 = 0; i9 < vCharInMap.size(); i9++)
			{
				((Char)vCharInMap.elementAt(i9)).update();
			}
			for (int i8 = 0; i8 < Teleport.vTeleport.size(); i8++)
			{
				((Teleport)Teleport.vTeleport.elementAt(i8)).update();
			}
			Char.myCharz().update();
			if (Char.myCharz().statusMe == 1)
			{
			...
			for (int i7 = 0; i7 < vMob.size(); i7++)
			{
				((Mob)vMob.elementAt(i7)).update();
			}
			GameCanvas.debug("E6", 0);
			for (int i6 = 0; i6 < vNpc.size(); i6++)
			{
				((Npc)vNpc.elementAt(i6)).update();
			}
			nSkill = onScreenSkill.Length;
			for (int i5 = onScreenSkill.Length - 1; i5 >= 0; i5--)
			{
				Skill skill = onScreenSkill[i5];
				if (skill != null)
				{
					nSkill = i5 + 1;
					break;
			...
			{
				((ItemMap)vItemMap.elementAt(i4)).update();
			}
			...
			for (int i2 = 0; i2 < Effect2.vEffect2.size(); i2++)
			{
				Effect2 effect = (Effect2)Effect2.vEffect2.elementAt(i2);
				effect.update();
			}
			for (int n = 0; n < Effect2.vEffect2Outside.size(); n++)
			{
				Effect2 effect2 = (Effect2)Effect2.vEffect2Outside.elementAt(n);
				effect2.update();
			}
			for (int m = 0; m < Effect2.vAnimateEffect.size(); m++)
			{
				Effect2 effect3 = (Effect2)Effect2.vAnimateEffect.elementAt(m);
				effect3.update();
			}
			for (int l = 0; l < Effect2.vEffectFeet.size(); l++)
			{
				Effect2 effect4 = (Effect2)Effect2.vEffectFeet.elementAt(l);
				effect4.update();
			}
			for (int k = 0; k < Effect2.vEffect3.size(); k++)
			{
				Effect2 effect5 = (Effect2)Effect2.vEffect3.elementAt(k);
				effect5.update();
			}
			...
			{
				((ItemTime)Char.vItemTime.elementAt(j)).update();
			}
	...
	public void paintEffect(mGraphics g)
	{
		...
		{
			for (int k = 0; k < Effect2.vAnimateEffect.size(); k++)
			{
				Effect2 effect2 = (Effect2)Effect2.vAnimateEffect.elementAt(k);
				effect2.paint(g);
			}
		}
		for (int j = 0; j < Effect2.vEffect2Outside.size(); j++)
		{
			Effect2 effect3 = (Effect2)Effect2.vEffect2Outside.elementAt(j);
			effect3.paint(g);
		}
	}
	...
	public override void paint(mGraphics g)
	{
		...
		for (int i2 = 0; i2 < vMob.size(); i2++)
		{
			((Mob)vMob.elementAt(i2)).paint(g);
		}
		for (int i3 = 0; i3 < Teleport.vTeleport.size(); i3++)
		{
			((Teleport)Teleport.vTeleport.elementAt(i3)).paint(g);
		}
		for (int i4 = 0; i4 < vCharInMap.size(); i4++)
		{
			Char char3 = null;
			try
			{
				char3 = (Char)vCharInMap.elementAt(i4);
			}
			catch (Exception)
			{
			}
		...
		for (int i5 = 0; i5 < vCharInMap.size(); i5++)
		{
			Char char4 = null;
			try
			{
				char4 = (Char)vCharInMap.elementAt(i5);
			}
			catch (Exception ex2)
			{
		...
		{
			((ItemMap)vItemMap.elementAt(i6)).paint(g);
		}
		NRO_v247.Mods.ModManager.OnPaint(g);
		g.translate(0, -GameCanvas.transY);
		...
		// paintEffect(g);
		// paintBgItem(g, 3);
		for (int i7 = 0; i7 < vNpc.size(); i7++)
		{
			Npc npc2 = (Npc)vNpc.elementAt(i7);
			npc2.paintName(g);
		}
		// EffecMn.paintLayer3(g);
		for (int i8 = 0; i8 < vNpc.size(); i8++)
		{
			Npc npc3 = (Npc)vNpc.elementAt(i8);
		...
		for (int i9 = 0; i9 < vCharInMap.size(); i9++)
		{
			...
			try
			{
				char5 = (Char)vCharInMap.elementAt(i9);
			}
			catch (Exception)
			{
			}
		...
		for (int i13 = 0; i13 < BackgroudEffect.vBgEffect.size(); i13++)
		{
			BackgroudEffect backgroudEffect = (BackgroudEffect)BackgroudEffect.vBgEffect.elementAt(i13);
		...
		if (!isPaintOther)
		{
			...
			*/
			...
			if ((TileMap.mapID == 128 || TileMap.mapID == 127) && mabuPercent != 0)
			{
				...
				int num7 = 200;
				g.setColor(0);
				g.fillRect(num6 - 27, num7 - 112, 54, 8);
				g.setColor(16711680);
				g.setClip(num6 - 25, num7 - 110, mabuPercent, 4);
				g.fillRect(num6 - 25, num7 - 110, 50, 4);
				g.setClip(0, 0, 3000, 3000);
				mFont.tahoma_7b_white.drawString(g, "Mabu", num6, num7 - 112 + 10, 2, mFont.tahoma_7b_dark);
			}
			...
			for (int i14 = 0; i14 < vCharInMap.size(); i14++)
			{
				Char char6 = null;
				try
				{
					char6 = (Char)vCharInMap.elementAt(i14);
				}
				catch (Exception)
				{
				}
			...
			resetTranslate(g);
			int num8 = 100 + ((Char.vItemTime.size() != 0) ? (textTime.size() * 12) : 0);
			if (Char.myCharz().clan != null)
			{
				int num9 = 0;
				int num10 = 0;
				int num2 = (GameCanvas.h - 100 - 60) / 12;
				for (int i15 = 0; i15 < vCharInMap.size(); i15++)
				{
					Char char7 = (Char)vCharInMap.elementAt(i15);
					...
					if (char7.isOutX() && char7.cx < Char.myCharz().cx)
					{
						...
						if (num9 <= num3)
						{
							mFont.tahoma_7_green.drawString(g, char7.cName, 20, num8 - 12 + num9 * 12, mFont.LEFT, mFont.tahoma_7_grey);
							char7.paintHp(g, 10, num8 + num9 * 12 - 5);
							num9++;
						}
					}
					else if (char7.isOutX() && char7.cx > Char.myCharz().cx && num10 <= num2)
					{
						mFont.tahoma_7_green.drawString(g, char7.cName, GameCanvas.w - 25, num8 - 12 + num10 * 12, mFont.RIGHT, mFont.tahoma_7_grey);
						char7.paintHp(g, GameCanvas.w - 15, num8 + num10 * 12 - 5);
						num10++;
					}
				}
			}
			...
			for (int i12 = 0; i12 < Char.vItemTime.size(); i12++)
			{
				((ItemTime)Char.vItemTime.elementAt(i12)).paint(g, cmdMenu.x + 32 + i12 * 24, 55);
			}
			for (int i11 = 0; i11 < textTime.size(); i11++)
			{
				((ItemTime)textTime.elementAt(i11)).paintText(g, cmdMenu.x + ((Char.vItemTime.size() == 0) ? 25 : 5), ((Char.vItemTime.size() == 0) ? 45 : 90) + i11 * 12);
			}
			paintXoSo(g);
			...
			{
				for (int i10 = 0; i10 < strPaint.Length; i10++)
				{
					mFont.tahoma_7b_white.drawString(g, strPaint[i10], 5, 85 + i10 * 18, 0, mFont.tahoma_7b_dark);
				}
			}
		}
		int num4 = 0;
		int num5 = GameCanvas.hw;
		if (num5 > 200)
		{
			num5 = 200;
		}
		paintPhuBanBar(g, num4 + GameCanvas.w / 2, 0, num5);
		...
		if (nCT_timeBallte > mSystem.currentTimeMillis() && TileMap.mapID == 170 && isPaint_CT && nCT_nBoyBaller / 2 > 0)
		{
			try
			{
				paint_CT(g, num4 + GameCanvas.w / 2, 0, num5);
			}
			catch (Exception)
			{
			}
	...
	private void paintImageBar(mGraphics g, bool isLeft, Char c)
	{
		if (c != null)
		{
			...
			g.setClip(0, 0, GameCanvas.w, GameCanvas.h);
			// Hiển thị % HP trên thanh máu
			int hpPercent = (c.cHPFull > 0) ? (int)(c.cHP * 100 / c.cHPFull) : 0;
			int hpBarCenterX = (int)(83 + hpBarW / 2);
			mFont.tahoma_7_white.drawString(g, hpPercent + "%", hpBarCenterX, 5, mFont.CENTER, mFont.tahoma_7b_dark);
			// Hiển thị % MP trên thanh xanh
			int mpPercent = (c.cMPFull > 0) ? (int)(c.cMP * 100 / c.cMPFull) : 0;
			int mpBarCenterX = (int)(83 + mpBarW / 2);
			mFont.tahoma_7_white.drawString(g, mpPercent + "%", mpBarCenterX, 20, mFont.CENTER, mFont.tahoma_7b_dark);
		...
		else
		{
			...
			else if (Char.myCharz().statusMe != 14)
			{
				if (gamePad.isSmallGamePad)
				{
					if (isAnalog != 1)
					{
						...
						else if (isudungCapsun4)
						{
							...
							SmallImage.drawSmallImage(g, 1088, xHP + 20, yHP - 6 - 40 + 25, 0, mGraphics.HCENTER | mGraphics.VCENTER);
						}
						else if (isudungCapsun3)
						{
							g.drawImage((mScreen.keyTouch != 14) ? imgNutF : imgNut, xHP + 5, yHP - 6 - 40 + 10, 0);
							SmallImage.drawSmallImage(g, 1087, xHP + 20, yHP - 6 - 40 + 25, 0, mGraphics.HCENTER | mGraphics.VCENTER);
						}
					}
					else if (isAnalog == 1)
					{
						...
						else if (isudungCapsun4)
						{
							...
							SmallImage.drawSmallImage(g, 1088, xHP + 20 + 5, yHP + 20 - 6 - 40 + 10, 0, mGraphics.HCENTER | mGraphics.VCENTER);
						}
						else if (isudungCapsun3)
						{
							g.drawImage((mScreen.keyTouch != 14) ? imgNut : imgNutF, xHP + 20 + 5, yHP + 20 - 6 - 40 + 10, mGraphics.HCENTER | mGraphics.VCENTER);
							SmallImage.drawSmallImage(g, 1087, xHP + 20 + 5, yHP + 20 - 6 - 40 + 10, 0, mGraphics.HCENTER | mGraphics.VCENTER);
						}
					}
				}
				else if (isAnalog != 1)
				{
					...
					else if (isudungCapsun4)
					{
						...
						SmallImage.drawSmallImage(g, 1088, xHP + 20, yHP + 20 - 6 - 40, 0, mGraphics.HCENTER | mGraphics.VCENTER);
					}
					else if (isudungCapsun3)
					{
						g.drawImage((mScreen.keyTouch != 14) ? imgNut : imgNutF, xHP + 20, yHP + 20 - 6 - 40, mGraphics.HCENTER | mGraphics.VCENTER);
						SmallImage.drawSmallImage(g, 1087, xHP + 20, yHP + 20 - 6 - 40, 0, mGraphics.HCENTER | mGraphics.VCENTER);
					}
				}
				else
				{
			...
			if (isHaveSelectSkill)
			{
				...
				for (int i = 0; i < num5; i++)
				{
					if (Main.isPC)
					{
						string[] array2 = ((!TField.isQwerty) ? new string[5] { "7", "8", "9", "10", "11" } : new string[10] { "1", "2", "3", "4", "5", "6", "7", "8", "9", "0" });
						int num6 = -13;
						if (num5 > 5 && i < 5)
						{
							num6 = 27;
					...
					{
						string[] array3 = ((!TField.isQwerty) ? new string[5] { "7", "8", "9", "1", "3" } : new string[5] { "Q", "W", "E", "R", "T" });
						g.setColor(11152401);
	...
	public void openUIZone(Message message)
	{
		...
		if (NRO_v247.Mods.AutoTrainFeature.IsRequestingZoneList)
		{
			NRO_v247.Mods.AutoTrainFeature.IsRequestingZoneList = false;
			NRO_v247.Mods.AutoTrainFeature.TryAutoSwitchZone();
			return;
		}
	...
	public void chatVip(string chatVip)
	{
		...
		NRO_v247.Mods.Notifications.NotifyCatcher.CatchVipChat(chatVip);
	}
	...
	public void paintPhuBanBar(mGraphics g, int x, int y, int w)
	{
		...
		PanelG.setTextColor(phuban_Info.color_1, 1).drawString(g, phuban_Info.nameTeam1, x - 5, num + 5, 1);
		PanelG.setTextColor(phuban_Info.color_2, 1).drawString(g, phuban_Info.nameTeam2, x + 5, num + 5, 0);
	...
	public void paint_CT(mGraphics g, int x, int y, int w)
	{
		...
		int frameHeight = fra_PVE_Bar_0.frameHeight;
		int num5 = y + frameHeight + mGraphics.getImageHeight(imgBall) / 2 + 2;
		int frameWidth = fra_PVE_Bar_1.frameWidth;
		int num6 = w / 2 - frameWidth / 2;
		int num7 = x - w / 2 + 3;
		int num8 = x + frameWidth / 2;
		int num9 = y + 3;
		int num10 = num6 - fra_PVE_Bar_0.frameWidth;
		int num11 = num10 / fra_PVE_Bar_0.frameWidth;
		if (num10 % fra_PVE_Bar_0.frameWidth > 0)
		{
			num11++;
		}
		for (int i = 0; i < num11; i++)
		{
			if (i < num11 - 1)
			{
				g.drawRegion(img_ct_bar_0, 0, 15, mGraphics.getImageWidth(img_ct_bar_0), 15, 2, num7 + fra_PVE_Bar_0.frameWidth + i * fra_PVE_Bar_0.frameWidth, num9, mGraphics.TOP | mGraphics.LEFT);
			}
			else
			{
				g.drawRegion(img_ct_bar_0, 0, 15, mGraphics.getImageWidth(img_ct_bar_0), 15, 2, num7 + num10, num9, mGraphics.TOP | mGraphics.LEFT);
			}
			if (i < num11 - 1)
			{
				g.drawRegion(img_ct_bar_0, 0, 15, mGraphics.getImageWidth(img_ct_bar_0), 15, 2, num8 + i * fra_PVE_Bar_0.frameWidth, num9, mGraphics.TOP | mGraphics.LEFT);
			}
			else
			{
				g.drawRegion(img_ct_bar_0, 0, 15, mGraphics.getImageWidth(img_ct_bar_0), 15, 2, num8 + num10 - fra_PVE_Bar_0.frameWidth, num9, mGraphics.TOP | mGraphics.LEFT);
			}
		}
		fra_PVE_Bar_0.drawFrame(0, num7, num9, 2, 0, g);
		fra_PVE_Bar_0.drawFrame(0, num8 + num10, num9, 0, 0, g);
		int num2 = nCT_TeamA * 100 / (nCT_nBoyBaller / 2) * num / 100;
		if (num2 > 0)
		{
			if (num2 < 6)
			{
		...
		if (nCT_TeamA > 0)
		{
			for (int j = 0; j < num4; j++)
			{
				if (j == 0)
				{
					g.drawRegion(img_ct_bar_0, 0, 60, mGraphics.getImageWidth(img_ct_bar_0), 15, 2, num7, num9, mGraphics.TOP | mGraphics.LEFT);
				}
				else
				{
					g.drawRegion(img_ct_bar_0, 0, 75, mGraphics.getImageWidth(img_ct_bar_0), 15, 2, num7 + j * 6, num9, mGraphics.TOP | mGraphics.LEFT);
				}
			}
		}
		GameCanvas.resetTrans(g);
		int num3 = nCT_TeamB * 100 / (nCT_nBoyBaller / 2) * num / 100;
		if (num - (num - num3) > 0)
		{
			if (num3 < 6)
			{
			...
			g.setClip(num8 + num - num3, num9, num - (num - num3), 15);
		}
		if (nCT_TeamB > 0)
		{
			for (int k = 0; k < num4; k++)
			{
				if (k == 0)
				{
					g.drawRegion(img_ct_bar_0, 0, 30, mGraphics.getImageWidth(img_ct_bar_0), 15, 0, num8 + num10, num9, mGraphics.TOP | mGraphics.LEFT);
				}
				else
				{
					g.drawRegion(img_ct_bar_0, 0, 45, mGraphics.getImageWidth(img_ct_bar_0), 15, 0, num8 + num10 - k * 6, num9, mGraphics.TOP | mGraphics.LEFT);
				}
			}
		}
		GameCanvas.resetTrans(g);
		...
		mFont.tahoma_7b_yellow.drawString(g, st, num7 + w / 2 - 2, y + 5, 2);
		mFont.tahoma_7_grey.drawString(g, "Tầng " + nCT_floor, num7 + w / 2 - 3, y + fra_PVE_Bar_1.frameHeight, mFont.CENTER);
		int width = mFont.tahoma_7b_red.getWidth(nCT_TeamA + string.Empty);
		mFont.tahoma_7b_blue.drawString(g, nCT_TeamA + string.Empty, x - frameWidth / 2 - width, num9 + fra_PVE_Bar_1.frameHeight, 0);
		SmallImage.drawSmallImage(g, 2325, x - frameWidth / 2 - width - 15, num9 + fra_PVE_Bar_1.frameHeight, 2, mGraphics.TOP | mGraphics.LEFT);
		width = mFont.tahoma_7b_red.getWidth(nCT_TeamB + string.Empty);
		mFont.tahoma_7b_red.drawString(g, nCT_TeamB + string.Empty, x + frameWidth / 2, num9 + fra_PVE_Bar_1.frameHeight, 0);
		SmallImage.drawSmallImage(g, 2323, x + frameWidth / 2 + width + 3, num9 + fra_PVE_Bar_1.frameHeight, 0, mGraphics.TOP | mGraphics.LEFT);
