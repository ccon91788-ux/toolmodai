public class Item
{
	...
	public void paintUpgradeEffect(int x, int y, int upgrade, mGraphics g)
	{
		...
		int num6 = 0;
		int num7 = ((upgrade >= 4) ? ((upgrade < 8) ? 1 : ((upgrade < 12) ? 2 : ((upgrade > 14) ? 4 : 3))) : 0);
		for (int i = num6; i < size.Length; i++)
		{
			int num8 = x - num / 2 + upgradeEffectX(GameCanvas.gameTick - i * 4);
			int num9 = y - num / 2 + upgradeEffectY(GameCanvas.gameTick - i * 4);
			g.setColor(colorBorder[num7][i]);
			g.fillRect(num8 - size[i] / 2, num9 - size[i] / 2, size[i], size[i]);
		}
		if (upgrade == 4 || upgrade == 8)
		{
			for (int j = num6; j < size.Length; j++)
			{
				int num10 = x - num / 2 + upgradeEffectX(GameCanvas.gameTick - num * 2 - j * 4);
				int num11 = y - num / 2 + upgradeEffectY(GameCanvas.gameTick - num * 2 - j * 4);
				g.setColor(colorBorder[num7 - 1][j]);
				g.fillRect(num10 - size[j] / 2, num11 - size[j] / 2, size[j], size[j]);
			}
		}
		if (upgrade != 1 && upgrade != 4 && upgrade != 8)
		{
			for (int k = num6; k < size.Length; k++)
			{
				int num12 = x - num / 2 + upgradeEffectX(GameCanvas.gameTick - num * 2 - k * 4);
				int num13 = y - num / 2 + upgradeEffectY(GameCanvas.gameTick - num * 2 - k * 4);
				g.setColor(colorBorder[num7][k]);
				g.fillRect(num12 - size[k] / 2, num13 - size[k] / 2, size[k], size[k]);
			}
		}
		if (upgrade != 1 && upgrade != 4 && upgrade != 8 && upgrade != 12 && upgrade != 2 && upgrade != 5 && upgrade != 9)
		{
			for (int l = num6; l < size.Length; l++)
			{
				int num2 = x - num / 2 + upgradeEffectX(GameCanvas.gameTick - num - l * 4);
				int num3 = y - num / 2 + upgradeEffectY(GameCanvas.gameTick - num - l * 4);
				g.setColor(colorBorder[num7][l]);
				g.fillRect(num2 - size[l] / 2, num3 - size[l] / 2, size[l], size[l]);
			}
		}
		...
		{
			for (int m = num6; m < size.Length; m++)
			{
				int num4 = x - num / 2 + upgradeEffectX(GameCanvas.gameTick - num * 3 - m * 4);
				int num5 = y - num / 2 + upgradeEffectY(GameCanvas.gameTick - num * 3 - m * 4);
				g.setColor(colorBorder[num7][m]);
				g.fillRect(num4 - size[m] / 2, num5 - size[m] / 2, size[m], size[m]);
			}
		}
	}
	...
	public int GetQuantity()
	{
		if (itemOption != null)
		{
			...
			{
				if (itemOption[i].optionTemplate.id == 31)
				{
					return itemOption[i].param;
				}
			}
		}
		return quantity;
