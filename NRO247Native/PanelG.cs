...
public class PanelG : IActionListener, IChatable
{
	...
	public void addItemDetail(Item item)
	{
		try
		{
			...
			if (item.itemOption != null)
			{
				for (int k = 0; k < item.itemOption.Length; k++)
				{
					...
					else
					{
						...
						NRO_v247.Mods.Items.SkhReportFeature.TryReportSkhTimeFromText(empty);
			...
			{
				string text5 = mResources.pow_request + ": " + currItem.template.strRequire;
		...
		try
		{
			...
			for (int i = 0; i < num; i++)
			{
				...
				if (item != null)
				{
					...
					{
						mFont.tahoma_7_yellow.drawString(g, string.Empty + item.GetQuantity(), num5 + num7, num6 + num8 - mFont.tahoma_7_yellow.getHeight(), 1);
					}
	...
	private void paintSkill(mGraphics g)
	{
		...
		for (int i = 0; i < num + 6; i++)
		{
			...
			int h = ITEM_HEIGHT - 1;
			int num9 = xScroll;
			int num10 = yScroll + i * ITEM_HEIGHT;
			int num11 = 34;
			int num12 = ITEM_HEIGHT - 1;
			...
			g.fillRect(num6, num7, num8, h);
			g.drawImage(GameScr.imgSkill, num9, num10, 0);
			if (i == 0)
			{
				SmallImage.drawSmallImage(g, 567, num9 + 4, num10 + 4, 0, 0);
				string st = mResources.HP + " " + mResources.root + ": " + NinjaUtil.getMoneys(Char.myCharz().cHPGoc);
				mFont.tahoma_7b_blue.drawString(g, st, num6 + 5, num7 + 3, 0);
				mFont.tahoma_7_green2.drawString(g, NinjaUtil.getMoneys(Char.myCharz().cHPGoc + 1000) + " " + mResources.potential + ": " + mResources.increase + " " + Char.myCharz().hpFrom1000TiemNang, num6 + 5, num7 + 15, 0);
			}
			if (i == 1)
			{
				SmallImage.drawSmallImage(g, 569, num9 + 4, num10 + 4, 0, 0);
				string st2 = mResources.KI + " " + mResources.root + ": " + NinjaUtil.getMoneys(Char.myCharz().cMPGoc);
				mFont.tahoma_7b_blue.drawString(g, st2, num6 + 5, num7 + 3, 0);
				mFont.tahoma_7_green2.drawString(g, NinjaUtil.getMoneys(Char.myCharz().cMPGoc + 1000) + " " + mResources.potential + ": " + mResources.increase + " " + Char.myCharz().mpFrom1000TiemNang, num6 + 5, num7 + 15, 0);
			}
			if (i == 2)
			{
				SmallImage.drawSmallImage(g, 568, num9 + 4, num10 + 4, 0, 0);
				string st3 = mResources.hit_point + " " + mResources.root + ": " + NinjaUtil.getMoneys(Char.myCharz().cDamGoc);
				mFont.tahoma_7b_blue.drawString(g, st3, num6 + 5, num7 + 3, 0);
				mFont.tahoma_7_green2.drawString(g, NinjaUtil.getMoneys(Char.myCharz().cDamGoc * 100) + " " + mResources.potential + ": " + mResources.increase + " " + Char.myCharz().damFrom1000TiemNang, num6 + 5, num7 + 15, 0);
			}
			if (i == 3)
			{
				SmallImage.drawSmallImage(g, 721, num9 + 4, num10 + 4, 0, 0);
				string st4 = mResources.armor + " " + mResources.root + ": " + NinjaUtil.getMoneys(Char.myCharz().cDefGoc);
				mFont.tahoma_7b_blue.drawString(g, st4, num6 + 5, num7 + 3, 0);
				mFont.tahoma_7_green2.drawString(g, NinjaUtil.getMoneys(500000 + Char.myCharz().cDefGoc * 100000) + " " + mResources.potential + ": " + mResources.increase + " " + Char.myCharz().defFrom1000TiemNang, num6 + 5, num7 + 15, 0);
			}
			if (i == 4)
			{
				SmallImage.drawSmallImage(g, 719, num9 + 4, num10 + 4, 0, 0);
				string st5 = mResources.critical + " " + mResources.root + ": " + Char.myCharz().cCriticalGoc + "%";
				long num13 = 50000000L;
				int num2 = Char.myCharz().cCriticalGoc;
				if (num2 > t_tiemnang.Length - 1)
				{
					num2 = t_tiemnang.Length - 1;
				}
				num13 = t_tiemnang[num2];
				mFont.tahoma_7b_blue.drawString(g, st5, num6 + 5, num7 + 3, 0);
				long number = num13;
				mFont.tahoma_7_green2.drawString(g, Res.formatNumber2(number) + " " + mResources.potential + ": " + mResources.increase + " " + Char.myCharz().criticalFrom1000Tiemnang, num6 + 5, num7 + 15, 0);
			}
			if (i == 5)
			{
				if (specialInfo != null)
				{
					SmallImage.drawSmallImage(g, spearcialImage, num9 + 4, num10 + 4, 0, 0);
					string[] array = mFont.tahoma_7.splitFontArray(specialInfo, 120);
					for (int j = 0; j < array.Length; j++)
					{
						mFont.tahoma_7_green2.drawString(g, array[j], num6 + 5, num7 + 3 + j * 12, 0);
					}
				}
				else
				{
					mFont.tahoma_7_green2.drawString(g, string.Empty, num6 + 5, num7 + 9, 0);
				}
			}
			if (i < 6)
			{
			...
			int num3 = i - 6;
			SkillTemplate skillTemplate = Char.myCharz().nClass.skillTemplates[num3];
			SmallImage.drawSmallImage(g, skillTemplate.iconId, num9 + 4, num10 + 4, 0, 0);
			Skill skill = Char.myCharz().getSkill(skillTemplate);
			if (skill != null)
			{
				mFont.tahoma_7b_blue.drawString(g, skillTemplate.name, num6 + 5, num7 + 3, 0);
				mFont.tahoma_7_blue.drawString(g, mResources.level + ": " + skill.point, num6 + num8 - 5, num7 + 3, mFont.RIGHT);
				if (skill.point == skillTemplate.maxPoint)
				{
					mFont.tahoma_7_green2.drawString(g, mResources.max_level_reach, num6 + 5, num7 + 15, 0);
				}
				else if (skill.template.isSkillSpec())
				{
					string text = mResources.proficiency + ": ";
					int x = mFont.tahoma_7_green2.getWidthExactOf(text) + num6 + 5;
					int num4 = num7 + 15;
					mFont.tahoma_7_green2.drawString(g, text, num6 + 5, num4, 0);
					mFont.tahoma_7_green2.drawString(g, "(" + skill.strCurExp() + ")", num6 + num8 - 5, num4, mFont.RIGHT);
					num4 += 4;
					g.setColor(7169134);
					g.fillRect(x, num4, 50, 5);
					int num5 = skill.curExp * 50 / 1000;
					g.setColor(11992374);
					g.fillRect(x, num4, num5, 5);
				...
				else
				{
					...
					mFont.tahoma_7_green2.drawString(g, mResources.level + " " + (skill.point + 1) + " " + mResources.need + " " + Res.formatNumber2(skill2.powRequire) + " " + mResources.potential, num6 + 5, num7 + 15, 0);
				}
			}
			else
			{
				...
				mFont.tahoma_7b_green.drawString(g, skillTemplate.name, num6 + 5, num7 + 3, 0);
				mFont.tahoma_7_green2.drawString(g, st6, num6 + 5, num7 + 15, 0);
			}
		}
		paintScrollArrow(g);
	}
		...
		try
		{
			...
			for (int i = 0; i < currentListLength; i++)
			{
				...
				if (i == 0)
				{
					for (int j = 0; j < num; j++)
					{
						int num10 = ((j == newSelected && selected == 0) ? ((GameCanvas.gameTick % 10 < 7) ? (-1) : 0) : 0);
						g.setColor((j != newSelected) ? 15723751 : 16383818);
						g.fillRect(xScroll + j * TAB_W_NEW, num4 + 9 + num10, TAB_W_NEW - 1, 14);
						mFont.tahoma_7_grey.drawString(g, string.Empty + j, xScroll + j * TAB_W_NEW + TAB_W_NEW / 2, yScroll + 11 + num10, mFont.CENTER);
					}
					continue;
				}
				g.setColor((i != selected) ? 15196114 : 16383818);
				...
				if (item.itemOption != null)
				{
					for (int n = 0; n < item.itemOption.Length; n++)
					{
						paintOptItem(g, item.itemOption[n].optionTemplate.id, item.itemOption[n].param, num6, num7, num8, num9);
					}
					for (int num2 = 0; num2 < item.itemOption.Length; num2++)
					{
						paintOptSlotItem(g, item.itemOption[num2].optionTemplate.id, item.itemOption[num2].param, num6, num7, num8, num9);
					}
				}
				if (item.quantity > 1)
				{
	...
	private void paintClans(mGraphics g)
	{
		...
		{
			int num3 = xScroll;
			int num4 = yScroll + j * ITEM_HEIGHT;
			int num5 = 24;
			int num6 = ITEM_HEIGHT - 1;
			int num7 = xScroll + num5;
			...
			case 0:
			{
				for (int k = 0; k < clansOption.Length; k++)
				{
					...
					g.fillRect(num + k * TAB_W, num8, TAB_W - 1, 23);
			...
			case 1:
				...
				g.fillRect(xScroll, num8, wScroll, num10);
				if (clanInfo != null)
				{
					mFont.tahoma_7b_dark.drawString(g, clanInfo, xScroll + wScroll / 2, num8 + 6, mFont.CENTER);
				}
				continue;
			}
			if (isSearchClan)
			{
				...
				if (ClanImage.isExistClanImage(clans[j - 2].imgID))
				{
					...
					{
						SmallImage.drawSmallImage(g, ClanImage.getClanImage((short)clans[j - 2].imgID).idImage[0], num3 + num5 / 2, num4 + num6 / 2, 0, StaticObj.VCENTER_HCENTER);
					}
				}
				else
				{
				...
				mFont.tahoma_7b_green2.drawString(g, st, num7 + 5, num8, 0);
				g.setClip(num7, num8, num9 - 10, num10);
				mFont.tahoma_7_blue.drawString(g, clans[j - 2].slogan, num7 + 5, num8 + 11, 0);
				g.setClip(xScroll, yScroll + cmy, wScroll, hScroll);
				mFont.tahoma_7_green2.drawString(g, clans[j - 2].currMember + "/" + clans[j - 2].maxMember, num7 + num9 - 5, num8, mFont.RIGHT);
				continue;
			}
			if (isViewMember)
			{
				...
				{
					SmallImage.drawSmallImage(g, member.headICON, num3, num4, 0, 0);
				}
				else
				{
					Part part = GameScr.parts[member.head];
					SmallImage.drawSmallImage(g, part.pi[Char.CharInfo[0][0][0]].id, num3 + part.pi[Char.CharInfo[0][0][0]].dx, num4 + 3 + part.pi[Char.CharInfo[0][0][0]].dy, 0, 0);
				}
				g.setClip(xScroll, yScroll + cmy, wScroll, hScroll);
				...
				mFont2.drawString(g, member.name, num7 + 5, num8, 0);
				mFont.tahoma_7_blue.drawString(g, mResources.power + ": " + member.powerPoint, num7 + 5, num8 + 11, 0);
				SmallImage.drawSmallImage(g, 7223, num7 + num9 - 7, num8 + 12, 0, 3);
				mFont.tahoma_7_blue.drawString(g, string.Empty + member.clanPoint, num7 + num9 - 15, num8 + 6, mFont.RIGHT);
				continue;
			}
			...
			g.fillRect(num3, num4, num9 + num5, num10);
			clanMessage.paint(g, num3, num4);
			if (clanMessage.option == null)
			{
				continue;
			}
			int num2 = xScroll + wScroll - 2 - clanMessage.option.Length * 40;
			for (int m = 0; m < clanMessage.option.Length; m++)
			{
				if (m == cSelected && j == selected)
				{
					g.drawImage(GameScr.imgLbtnFocus2, num2 + m * 40 + 20, num8 + num10 / 2, StaticObj.VCENTER_HCENTER);
					mFont.tahoma_7b_green2.drawString(g, clanMessage.option[m], num2 + m * 40 + 20, num8 + 6, mFont.CENTER);
				}
				else
				{
					g.drawImage(GameScr.imgLbtn2, num2 + m * 40 + 20, num8 + num10 / 2, StaticObj.VCENTER_HCENTER);
					mFont.tahoma_7b_dark.drawString(g, clanMessage.option[m], num2 + m * 40 + 20, num8 + 6, mFont.CENTER);
				}
			}
		}
		paintScrollArrow(g);
	...
	private void paintInventory(mGraphics g)
	{
		...
		if (flag && isnewInventory)
		{
			...
			int num12 = num / 20 + ((num % 20 > 0) ? 1 : 0) + 1;
			int num23 = 0;
			TAB_W_NEW = wScroll / num12;
			for (int i = num23; i < num12; i++)
			{
				int num29 = ((i == newSelected && selected == 0) ? ((GameCanvas.gameTick % 10 < 7) ? (-1) : 0) : 0);
				g.setColor((i != newSelected) ? 15723751 : 16383818);
				g.fillRect(xScroll + i * TAB_W_NEW, 89 + num29 - 10, TAB_W_NEW - 1, 21);
				if (i == newSelected)
				{
					g.setColor(13524492);
					int x = xScroll + i * TAB_W_NEW;
					int num30 = 89 + num29 - 10 + 21;
					g.fillRect(x, num30 - 3, TAB_W_NEW - 1, 3);
				}
				mFont.tahoma_7_grey.drawString(g, string.Empty + (i + 1), xScroll + i * TAB_W_NEW + TAB_W_NEW / 2, 91 + num29 - 10, mFont.CENTER);
			}
			num23 = 1;
			int num31 = xScroll;
			int num32 = yScroll + num23 * ITEM_HEIGHT;
			int num33 = 34;
			int num34 = ITEM_HEIGHT - 1;
			for (int j = 0; j < 4; j++)
			{
				num31 = xScroll;
				num32 = yScroll + (j + num23) * ITEM_HEIGHT;
				bool flag2 = true;
				for (int k = 0; k < 5; k++)
				{
					Item item = null;
					...
					if (newSelected > 0)
					{
						num2 = (newSelected - 1) * 20;
						if (j * 5 + k + num2 < arrItemBag.Length)
						{
							item = arrItemBag[j * 5 + k + num2];
							num31 = xScroll + num33 * k;
							int num3 = sellectInventory % 5;
							int num4 = sellectInventory / 5;
							if (newSelected > 0)
							{
								g.setColor(15196114);
							}
							...
							g.drawRect(num31, num32, num33, num34);
							if (j == num4 && k == num3 && selected > 0)
							{
								g.setColor(16383818);
								itemInvenNew = item;
							}
							g.fillRect(num31 + 2, num32 + 2, num33 - 3, num34 - 3);
							if (item != null)
							{
								int x2 = num31 + imgNew.getWidth() / 2;
								int y = num32;
								int num5 = 34;
								int h = ITEM_HEIGHT - 1;
								SmallImage.drawSmallImage(g, item.template.iconID, num31 + num33 / 2, num32 + num34 / 2, 0, 3);
								if (item.quantity > 1)
								{
									mFont.tahoma_7_yellow.drawString(g, string.Empty + item.quantity, num31, num32 - mFont.tahoma_7_yellow.getHeight(), 1);
								}
								if (item.newItem && GameCanvas.gameTick % 10 > 5)
								{
								...
								for (int l = 0; l < item.itemOption.Length; l++)
								{
									paintOptSlotItem(g, item.itemOption[l].optionTemplate.id, item.itemOption[l].param, x2, y, num5, h);
								}
							}
							if (!flag2)
							{
			...
			num23 = ((newSelected != 0) ? 5 : 3);
			int num6 = yScroll + num23 * ITEM_HEIGHT + 5;
			int num7 = 2;
			if (newSelected == 0)
			{
				num7 = 4;
			...
			num31 = xScroll;
			num32 = yScroll + num23 * ITEM_HEIGHT;
			num33 = 34;
			num34 = ITEM_HEIGHT - 1;
			if (newSelected == 0)
			{
				...
				{
					eBanner.paint(g);
					eBanner.x = num31 + 34 + 34;
					eBanner.y = num32 + num34 - 25;
				}
				for (int m = 0; m < 10; m++)
				{
					Item item2 = null;
					...
					if (m < 5)
					{
						num8 = 0;
						num31 = xScroll;
						num32 = yScroll + (m + num23) * ITEM_HEIGHT;
					}
					else
					{
						num8 = 5;
						num31 = xScroll + 4 * num33;
						num32 = yScroll + (m - num8 + num23) * ITEM_HEIGHT;
					}
					g.setColor(15196114);
					g.drawRect(num31, num32, num33, num34);
					...
					if (item2 == null)
					{
						screenTab6.drawFrame(m, num31 + num33 / 2 - 8, num32 + num34 / 2 - 8, 0, mGraphics.TOP | mGraphics.LEFT, g);
					}
					if (item2 != null)
					{
						SmallImage.drawSmallImage(g, item2.template.iconID, num31 + num33 / 2, num32 + num34 / 2, 0, 3);
						if (item2.quantity > 1)
						{
							mFont.tahoma_7_yellow.drawString(g, string.Empty + item2.quantity, num31 + 4 * num33, num32 - mFont.tahoma_7_yellow.getHeight(), 1);
						}
					}
				}
				num23 = 1;
				...
				num32 = yScroll + num23 * ITEM_HEIGHT;
				num33 = 102;
				num34 = 4 * (ITEM_HEIGHT - 1);
				Char.myCharz().paintCharBody(g, num31 + 34 + 17, num32 + num34 - 25, 1, 0, isPaintBag: true);
				num23 = 3;
				num7 = 2;
				num31 = xScroll + 34;
				num32 = yScroll + (1 + num23) * ITEM_HEIGHT - 1;
				num33 = 102;
				num34 = ITEM_HEIGHT * num7;
				g.setColor(15196114);
				g.drawRect(num31, num32, num33, num34);
				g.setColor(9993045);
				g.fillRect(num31 + 1, num32 + 1, num33 - 2, num34 - 2);
				paintItemBodyBagInfo(g, num31 + 3, num32 - 2);
				num23 = ((newSelected != 0) ? 5 : 6);
				num6 = yScroll + num23 * ITEM_HEIGHT;
				g.setColor(15196114);
				if (newSelected == 0)
				{
					num7 = 1;
				}
				g.drawRect(xScroll, num6, wScroll, ITEM_HEIGHT * num7);
				g.setColor(16777215);
				g.fillRect(xScroll + 1, num6 + 1, wScroll - 2, ITEM_HEIGHT * num7 - 2);
			}
			if (itemInvenNew != null && itemInvenNew.itemOption != null)
			{
				...
				mFont2.drawString(g, itemInvenNew.template.name + text, xScroll + 5, num6 + 1, 0);
				string text2 = string.Empty;
				if (itemInvenNew.itemOption != null)
				{
					...
					if (itemInvenNew.itemOption.Length > 1)
					{
						...
						{
							if (itemInvenNew.itemOption[num9] != null && itemInvenNew.itemOption[num9].optionTemplate.id != 102 && itemInvenNew.itemOption[num9].optionTemplate.id != 107)
							{
								text2 = text2 + "," + itemInvenNew.itemOption[num9].getOptionString();
							}
						}
					}
					try
					...
					mFont3.drawString(g, text2, xScroll + 5, num6 + 11, mFont.LEFT);
				}
			}
		}
		if (flag && isnewInventory)
		...
		int num10 = (arrItemBody2.Length + arrItemBag2.Length) / 20 + (((arrItemBody2.Length + arrItemBag2.Length) % 20 > 0) ? 1 : 0);
		TAB_W_NEW = wScroll / num10;
		for (int num11 = 0; num11 < num10; num11++)
		{
			int num13 = ((num11 == newSelected && selected == 0) ? ((GameCanvas.gameTick % 10 < 7) ? (-1) : 0) : 0);
			g.setColor((num11 != newSelected) ? 15723751 : 16383818);
			g.fillRect(xScroll + num11 * TAB_W_NEW, 89 + num13 - 10, TAB_W_NEW - 1, 21);
			if (num11 == newSelected)
			{
				g.setColor(13524492);
				int x3 = xScroll + num11 * TAB_W_NEW;
				int num14 = 89 + num13 - 10 + 21;
				g.fillRect(x3, num14 - 3, TAB_W_NEW - 1, 3);
			}
			mFont.tahoma_7_grey.drawString(g, string.Empty + (num11 + 1), xScroll + num11 * TAB_W_NEW + TAB_W_NEW / 2, 91 + num13 - 10, mFont.CENTER);
		}
		g.setClip(xScroll, yScroll + 21, wScroll, hScroll - 21);
		g.translate(0, -cmy);
		try
		{
			for (int num15 = 1; num15 < currentListLength; num15++)
			{
				int num16 = xScroll + 36;
				int num17 = yScroll + num15 * ITEM_HEIGHT;
				int num18 = wScroll - 36;
				int h2 = ITEM_HEIGHT - 1;
				int num19 = xScroll;
				int num20 = yScroll + num15 * ITEM_HEIGHT;
				int num21 = 34;
				int num22 = ITEM_HEIGHT - 1;
				if (num17 - cmy > yScroll + hScroll || num17 - cmy < yScroll - ITEM_HEIGHT)
				{
					continue;
				}
				bool inventorySelect_isbody = GetInventorySelect_isbody(num15, newSelected, Char.myCharz().arrItemBody);
				int inventorySelect_body = GetInventorySelect_body(num15, newSelected);
				int inventorySelect_bag = GetInventorySelect_bag(num15, newSelected, Char.myCharz().arrItemBody);
				g.setColor((num15 == selected) ? 16383818 : ((!inventorySelect_isbody) ? 15723751 : 15196114));
				g.fillRect(num16, num17, num18, h2);
				g.setColor((num15 == selected) ? 9541120 : ((!inventorySelect_isbody) ? 11837316 : 9993045));
				...
				if (item3 != null)
				{
					...
					{
						if (item3.itemOption[num24].optionTemplate.id == 72 && item3.itemOption[num24].param > 0)
						{
							byte id = (byte)GetColor_Item_Upgrade(item3.itemOption[num24].param);
							int color_ItemBg = GetColor_ItemBg(id);
							if (color_ItemBg != -1)
							{
								g.setColor((num15 != selected) ? GetColor_ItemBg(id) : GetColor_ItemBg(id));
							}
						}
					}
				}
				g.fillRect(num19, num20, num21, num22);
				if (item3 != null && item3.isSelect && GameCanvas.panel.type == 12)
				{
					g.setColor((num15 != selected) ? 6047789 : 7040779);
					g.fillRect(num19, num20, num21, num22);
				}
				if (item3 == null)
				{
				...
				{
					for (int num25 = 0; num25 < item3.itemOption.Length; num25++)
					{
						if (item3.itemOption[num25].optionTemplate.id == 72)
						{
							text3 = " [+" + item3.itemOption[num25].param + "]";
						}
						if (item3.itemOption[num25].optionTemplate.id == 41)
						{
							if (item3.itemOption[num25].param == 1)
							{
								mFont4 = GetFont(0);
							}
							else if (item3.itemOption[num25].param == 2)
							{
								mFont4 = GetFont(2);
							}
							else if (item3.itemOption[num25].param == 3)
							{
								mFont4 = GetFont(8);
							}
							else if (item3.itemOption[num25].param == 4)
							{
								mFont4 = GetFont(7);
							}
						}
				...
				mFont4.drawString(g, item3.template.name + text3, num16 + 5, num17 + 1, 0);
				...
				if (item3.itemOption != null)
				{
					...
					{
						for (int num26 = 1; num26 < 2; num26++)
						{
							if (item3.itemOption[num26] != null && item3.itemOption[num26].optionTemplate.id != 102 && item3.itemOption[num26].optionTemplate.id != 107)
							{
								text4 = text4 + "," + item3.itemOption[num26].getOptionString();
							}
						}
					}
					mFont5.drawString(g, text4, num16 + 5, num17 + 11, mFont.LEFT);
				}
				SmallImage.drawSmallImage(g, item3.template.iconID, num19 + num21 / 2, num20 + num22 / 2, 0, 3);
				if (item3.itemOption != null)
				{
					for (int num27 = 0; num27 < item3.itemOption.Length; num27++)
					{
						paintOptItem(g, item3.itemOption[num27].optionTemplate.id, item3.itemOption[num27].param, num19, num20, num21, num22);
					}
					for (int num28 = 0; num28 < item3.itemOption.Length; num28++)
					{
						paintOptSlotItem(g, item3.itemOption[num28].optionTemplate.id, item3.itemOption[num28].param, num19, num20, num21, num22);
					}
				}
				if (item3.quantity > 1)
				{
					mFont.tahoma_7_yellow.drawString(g, string.Empty + item3.quantity, num19 + num21, num20 + num22 - mFont.tahoma_7_yellow.getHeight(), 1);
				}
			}
		}
		catch (Exception)
	...
	private void paintTopInfo(mGraphics g)
	{
		...
		case 21:
			if (currentTabIndex == 0)
			{
				Debug.WriteLine(">>>head:" + Char.myPetz().avatarz());
	...
	public void perform(int idAction, object p)
	{
		...
		if (idAction == 6000)
		{
			...
			{
				Item item12 = (Item)GameCanvas.panel.vItemCombine.elementAt(i);
				if (item12.template.id == item11.template.id)
				{
			...
			GameCanvas.panel.vItemCombine.addElement(item11);
			if (GameCanvas.panel.currentTabIndex == 0)
			{
				GameCanvas.panel.setTabCombine();
			}
		...
		if (idAction == 7000)
		{
			...
			{
				Item item14 = (Item)GameCanvas.panel.vMyGD.elementAt(j);
				if (item14.indexUI == item13.indexUI)
				{
					GameCanvas.startOKDlg(mResources.already_has_item);
					return;
				}
			}
			if (item13.quantity > 1)
			{
				putQuantily();
				return;
			}
			item13.isSelect = true;
			Item item15 = new Item();
			item15.template = item13.template;
			item15.itemOption = item13.itemOption;
			item15.indexUI = item13.indexUI;
			GameCanvas.panel.vMyGD.addElement(item15);
			Service.gI().giaodich(2, -1, (sbyte)item15.indexUI, item15.quantity);
		}
		if (idAction == 7001)
		{
			Item item16 = (Item)p;
			item16.isSelect = false;
			GameCanvas.panel.vMyGD.removeElement(item16);
			if (GameCanvas.panel.currentTabIndex == 1)
			{
				GameCanvas.panel.setTabGiaoDich(isMe: true);
			}
			Service.gI().giaodich(4, -1, (sbyte)item16.indexUI, -1);
		}
		if (idAction == 7002)
		{
			isAccept = true;
		...
		if (idAction == 2010)
		{
			...
			Item item17 = (Item)p;
			if (item17 != null && (item17.template.id == 193 || item17.template.id == 194))
			{
				GameCanvas.panel.hide();
			}
		}
		...
		if (idAction == 2001)
		{
			...
			if (item18.template.id == 193 || item18.template.id == 194)
			{
				GameCanvas.panel.hide();
			}
		}
		...
		if (idAction == 3000)
		{
			Res.outz("mua do");
			Item item2 = (Item)p;
			Service.gI().buyItem(0, item2.template.id, 0);
		}
		if (idAction == 3001)
		{
			Item item3 = (Item)p;
			GameCanvas.msgdlg.pleasewait();
			Service.gI().buyItem(1, item3.template.id, 0);
		}
		if (idAction == 3002)
		{
			GameCanvas.endDlg();
		...
		{
			Item item4 = (Item)p;
			Service.gI().buyItem(3, item4.template.id, 0);
		}
		if (idAction == 3005)
		{
			Res.outz("mua do");
			Item item5 = (Item)p;
			Service.gI().buyItem(3, item5.template.id, 0);
		}
		if (idAction == 4000)
		{
			Clan clan = (Clan)p;
		...
		{
			Item item6 = (Item)p;
			Service.gI().kigui(1, item6.itemId, -1, -1, -1);
			InfoDlg.showWait();
		}
		if (idAction == 10015)
		{
			Item item7 = (Item)p;
			Service.gI().kigui(2, item7.itemId, -1, -1, -1);
			InfoDlg.showWait();
		}
		if (idAction == 10016)
		{
			Item item8 = (Item)p;
			Service.gI().kigui(3, item8.itemId, 0, item8.buyCoin, -1);
			InfoDlg.showWait();
		}
		if (idAction == 10017)
		{
			Item item9 = (Item)p;
			Service.gI().kigui(3, item9.itemId, 1, item9.buyGold, -1);
			InfoDlg.showWait();
		}
		if (idAction == 10018)
		{
			Item item10 = (Item)p;
			Service.gI().kigui(5, item10.itemId, -1, -1, -1);
			InfoDlg.showWait();
		}
		if (idAction == 10019)
		{
