...
public class ChatPopup : Effect2, IActionListener
{
	...
	public static void addChatPopupMultiLine(string chat, int howLong, Npc c)
	{
		...
		NRO_v247.Mods.Notifications.NotifyCatcher.CatchNpcMessage((c != null) ? c.cName : "", chat, c);
	}
	...
	public override void paint(mGraphics g)
	{
		...
		int num6 = sayWidth + 2;
		int num7 = ch;
		if ((num <= 0 || num5 <= 0) && !GameCanvas.panel.isShow)
		{
			return;
		}
		int num8 = 0;
		if (c != null)
		{
			num8 = ((GameCanvas.gameTick % 10 <= 2) ? 1 : 0);
			SmallImage.drawSmallImage(g, c.avatar, cx + 14, cy + num8, 0, StaticObj.BOTTOM_LEFT);
		}
		if (iconID != 0)
		{
			num8 = ((GameCanvas.gameTick % 10 <= 2) ? 1 : 0);
			SmallImage.drawSmallImage(g, iconID, cx + num6 / 2, cy + ch - 15 + num8, 0, StaticObj.VCENTER_HCENTER);
		}
		PopUp.paintPopUp(g, num, num5, num6, num7, 16777215, isButton: false);
		if (scr != null)
		{
			g.setClip(num, num5, num6, num7 - 16);
		...
		if (isClip)
		{
			tx = g.getTranslateX();
			ty = g.getTranslateY();
			g.setClip(num, num5 + 1, num6, num7 - 17);
		...
		for (int i = 0; i < says.Length; i++)
		{
			...
			{
				g.setColor(0);
				g.fillRect(num + 10, cy + sayRun + i * 12 + 6, num6 - 20, 1);
				continue;
			}
			...
			if (says[i].StartsWith("<"))
			{
				...
				st = second + " " + array3[2];
				mFont2.drawString(g, st, cx + sayWidth / 2, cy + sayRun + i * 12 - strY + 12, num10);
			}
			else
			{
				if (num10 == 2)
				...
				if (num10 == 1)
				{
					mFont2.drawString(g, st, cx + sayWidth - 5, cy + sayRun + i * 12 - strY + 12, num10);
				}
			}
		}
		if (isClip)
		...
		if (maxStarSlot > 4)
		{
			...
			{
				g.drawImage(PanelG.imgMaxStar, num + num6 / 2 - nMaxslot_tren * 20 / 2 + j * 20 + mGraphics.getImageWidth(PanelG.imgMaxStar), num5 + num7 - 17, 3);
				array4[j] = num + num6 / 2 - nMaxslot_tren * 20 / 2 + j * 20 + mGraphics.getImageWidth(PanelG.imgMaxStar);
				array5[j] = num5 + num7 - 17;
			}
			for (int k = 0; k < nMaxslot_duoi; k++)
			{
				g.drawImage(PanelG.imgMaxStar, num + num6 / 2 - nMaxslot_duoi * 20 / 2 + k * 20 + mGraphics.getImageWidth(PanelG.imgMaxStar), num5 + num7 - 8, 3);
				array4[nMaxslot_tren + k] = num + num6 / 2 - nMaxslot_duoi * 20 / 2 + k * 20 + mGraphics.getImageWidth(PanelG.imgMaxStar);
				array5[nMaxslot_tren + k] = num5 + num7 - 8;
			}
			if (maxStarSlot >= 7)
			{
				int num12 = 7;
				for (int l = 7; l < maxStarSlot; l++)
				{
					if (starCuongHoa[l])
					{
						g.drawImage(PanelG.imgStarCuongHoa, array4[l], array5[l], 3);
					}
				}
			}
			if (starSlot > 0)
			{
				imgStar = PanelG.imgStar;
				if (starSlot >= nMaxslot_tren)
				{
					nslot_duoi = starSlot - nMaxslot_tren;
					for (int m = 0; m < nMaxslot_tren; m++)
					{
						g.drawImage(imgStar, num + num6 / 2 - nMaxslot_tren * 20 / 2 + m * 20 + mGraphics.getImageWidth(imgStar), num5 + num7 - 17, 3);
					}
					for (int n = 0; n < nslot_duoi; n++)
					{
						if (n + nMaxslot_tren >= numSlot)
						{
							imgStar = PanelG.imgStar8;
						}
						g.drawImage(imgStar, num + num6 / 2 - nMaxslot_duoi * 20 / 2 + n * 20 + mGraphics.getImageWidth(imgStar), num5 + num7 - 8, 3);
					}
				}
				else
				{
					for (int num2 = 0; num2 < starSlot; num2++)
					{
						g.drawImage(imgStar, num + num6 / 2 - nMaxslot_tren * 20 / 2 + num2 * 20 + mGraphics.getImageWidth(imgStar), num5 + num7 - 17, 3);
					}
				}
			}
		}
		...
		{
			for (int num3 = 0; num3 < maxStarSlot; num3++)
			{
				g.drawImage(PanelG.imgMaxStar, num + num6 / 2 - maxStarSlot * 20 / 2 + num3 * 20 + mGraphics.getImageWidth(PanelG.imgMaxStar), num5 + num7 - 13, 3);
			}
			if (starSlot > 0)
			{
				for (int num4 = 0; num4 < starSlot; num4++)
				{
					g.drawImage(PanelG.imgStar, num + num6 / 2 - maxStarSlot * 20 / 2 + num4 * 20 + mGraphics.getImageWidth(PanelG.imgStar), num5 + num7 - 13, 3);
				}
			}
		}
		paintCmd(g);
