...
public class GameCanvas : IActionListener
{
	...
	public static PanelG panel;

	public static PanelG panel2;
	...
	public void initGame()
	{
		try
		{
			...
			PanelG.WIDTH_PANEL = 176;
			if (PanelG.WIDTH_PANEL > w)
			{
				PanelG.WIDTH_PANEL = w;
			}
...
//					GameMidlet.instance.exit();
			...
			if (currentScreen != null)
			{
				...
				// Hook AutoLogin vào GameCanvas
				NRO_v247.AutoLogin.gI().Update();
				NRO_v247.Mods.ModManager.Update();
...
//						AutoLogin.gI().update();
			...
			if (Controller.isConnectionFail)
			{
				if (!Controller.isMain)
				{
					if (currentScreen == serverScreen && ServerListScreen.isGetData && !Service.reciveFromMainSession)
					{
						...
						serverScreen.cancel();
						Debug.WriteLine("connect fail 1");
					}
					if (currentScreen == loginScr && !Service.reciveFromMainSession)
					{
						onConnectionFail();
						Debug.WriteLine("connect fail 2");
					}
				}
				else
				{
					...
					Debug.WriteLine("connect fail 3");
				}
			...
			if (currentScreen != null && currentScreen is GameScr)
			{
				...
				if (xThongBaoTranslate - PanelG.imgNew.getWidth() <= 60)
				{
					dir_ = 0;
					tickWaitThongBao++;
	...
	public void paint(mGraphics g)
	{
		try
		{
			...
			// Hook vẽ UI của Bot (luôn nằm trên cùng)
			try { NRO_v247.Mods.ModManager.OnPaintOverlay(g); } catch { }
		}
		catch (Exception)
		{
		}
	...
	public static void startOKDlg(string info)
	{
        ...
        NRO_v247.AutoLogin.CaptureServerMessage(info);
	}
	...
	public static void startWaitDlg(string info)
	{
		...
		currentDialog = msgdlg;
		msgdlg.isWait = true;
        NRO_v247.AutoLogin.CaptureServerMessage(info); // Fix: bắt message countdown/bảo trì
	}
	...
	public static Image loadImageRMS(string path)
	{
		...
		catch (Exception ex)
		{
			...
			{
				Debug.WriteLine("Loi ham khong tim thay a: " + ex.ToString());
				return result;
			}
		}
	}
