...
public class GamePad
{
	...
	public void update()
	{
		try
		{
			...
			if (GameCanvas.isPointerDown && !GameCanvas.isPointerJustRelease)
			{
				...
				delta = (int)(Math.Pow(deltaX, 2.0) + (double)(int)Math.Pow(deltaY, 2.0));
				d = Res.sqrt(delta);
	...
	public bool disableClickMove()
	{
		try
		{
			...
			bool flag = false;
			if ((GameCanvas.px >= xZone && GameCanvas.px <= xZone + wZone && GameCanvas.py >= yZone && GameCanvas.py <= yZone + hZone) || (GameCanvas.px >= xZone && GameCanvas.px <= GameCanvas.w && GameCanvas.py >= yZone && GameCanvas.py <= yZone + hZone) || GameCanvas.px >= GameCanvas.w - 50)
			{
				return true;
			}
			return false;
