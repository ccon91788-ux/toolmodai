...
public class DataInputStream
{
	...
	{
		byte[] textAsset = AssetBundle.GetBytes(filename);
		r = new myReader(ArrayCast.cast(textAsset));
	}
	...
	private static DataInputStream _getResourceAsStream(string filename)
	{
		if (status != 0)
		{
			...
			if (status != 0)
			{
				Debug.WriteLine("CANNOT GET INPUTSTREAM " + filename + " WHEN GETTING " + filenametemp);
				return null;
			}
		}
		istemp = null;
		...
		if (j == 500)
		{
			Debug.WriteLine("TOO LONG FOR CREATE INPUTSTREAM " + filename);
			status = 0;
			return null;
		}
		return istemp;
