...
namespace NRO_v247.Mods
{
    public partial class AutoTrainFeature : HotReloadFeatureBase<AutoTrainFeature.TrainPanelSettings>, IAutoFeature, ICleanupFeature
    {
        ...
        private bool _hasForcedAutoPlayPermission = false;
        private bool _previousCanAutoPlay = false;
        ...
        public void PauseTrainForAction()
        {
            ...
            RestoreAutoPlayPermissionIfNeeded();
        }
        ...
        public void Update()
        {
            ...
            // Fix #2: Cache TDLT 1 lần duy nhất mỗi frame, tránh duyệt danh sách buff 5+ lần
            TdltController.Update(UseTDLT);
            _isTdltActiveThisFrame = true;

            if (!EnsureMapAndZone())
                return;

            EnsureAutoPlayPermissionForTrain();
            GameScr.isAutoPlay = false;
        ...
        private void EnsureAutoPlayPermissionForTrain()
        {
            if (!_hasForcedAutoPlayPermission)
            {
                _previousCanAutoPlay = GameScr.canAutoPlay;
                _hasForcedAutoPlayPermission = true;
            }

            GameScr.canAutoPlay = true;
        }

        private void RestoreAutoPlayPermissionIfNeeded()
        {
            if (!_hasForcedAutoPlayPermission) return;
            GameScr.canAutoPlay = _previousCanAutoPlay;
            _hasForcedAutoPlayPermission = false;
        }
