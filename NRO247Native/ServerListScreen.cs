...
public class ServerListScreen : mScreen, IActionListener
{
	...
	public static string smartPhoneVN = "NRSD:server.ngocrongsaoden.com:14445:0:0:0,0,0";

	public static string javaVN = "NRSD:server.ngocrongsaoden.com:14445:0:0:0,0,0";
	...
	private void initCommand()
	{
		...
		for (int i = 0; i < cmd.Length; i++)
		{
			...
			case 0:
				...
				cmd[0].caption = "Tool by soulmate";
				break;
			case 1:
				if (nCmdPlay == 1)
				{
	...
	{
		if (NRO_v247.AutoLogin.IsEnabled && !string.IsNullOrEmpty(NRO_v247.AutoLogin.IP) && NRO_v247.AutoLogin.Port > 0)
		{
			nameServer = new string[1] { NRO_v247.AutoLogin.IP == "server.ngocrongsaoden.com" ? "NRSD" : NRO_v247.AutoLogin.IP };
			address = new string[1] { NRO_v247.AutoLogin.IP };
			port = new short[1] { (short)NRO_v247.AutoLogin.Port };
			language = new sbyte[1] { 0 };
			typeSv = new sbyte[1] { 0 };
			isNew = new sbyte[1] { 0 };
			hasConnected = new bool[2];
			serverPriority = 0;
			SetIpSelect(0, issave: false);
			ConnectIP();
			return;
		}
	...
	public void init()
	{
		...
		// Chỉ trigger download khi chưa có data (loadScreen = false)
		// Nếu loadScreen = true nghĩa là đã có ResVersion → không cần tải lại
		if (!loadScreen)
		{
			this.perform(2, (Object)null);
		}
	}

	public void show2()
	{
		Debug.WriteLine(">>>>ServerListScreen show2: ");
		GameScr.cmx = 0;
		GameScr.cmy = 0;
		initCommand();
		loadScreen = false;
	...
	{
		Debug.WriteLine(">>>>SetIpSelect: " + index + "  save:" + issave);
		ipSelect = index;
		if (issave)
		{
			Rms.saveRMSInt(RMS_svselect, ipSelect);
	...
	public void Login_New()
	{
		...
		if (!flag && !flag2)
		{
			if (text == null || text.Equals(string.Empty))
			{
				Debug.WriteLine(">>>>Login_New: login2: ");
				Service.gI().login2(string.Empty);
			}
			else
			{
	...
	public static void CheckBack_ServerListScreen()
	{
		...
		Debug.WriteLine(">>>>CheckBack_ServerListScreen: " + ipSelect + "  auto login:" + isAutoLogin);
