...
public class Mob : IMapObject
{
	...
	{
		if (ModBootstrap.TrainFeature != null && ModBootstrap.TrainFeature.IsTrain) return; // Tắt gây dame
	...
	{
		if (ModBootstrap.TrainFeature != null && ModBootstrap.TrainFeature.IsTrain) return; // không set gây dame tạm thời
	...
	public void updateMobWalk()
	{
		...
		try
		{
			if (ModBootstrap.TrainFeature != null && ModBootstrap.TrainFeature.IsTrain) return; // Tắt di chuyển
			if (injureThenDie)
			{
				status = 1;
				p2 = injureBy.cdir << 3;
