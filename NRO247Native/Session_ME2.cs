...
public class Session_ME2 : ISession
{
	...
	// ─── Proxy config (set via Panel PROXY_SETTING command) ───
	public static bool HasProxyConfigured = false;
	public static string ProxyHost = "";
	public static int ProxyPort = 0;
	public static int ProxyType = 0; // 0=HTTP, 1=SOCKS5
	...
	public void doConnect(string host, int port)
	{
		if (HasProxyConfigured)
		{
			...
			if (ProxyType == 1)
			{
				sc = Session_ME.ConnectViaSocks5Pub(ProxyHost, ProxyPort, host, port, ProxyUsername, ProxyPassword);
				dataStream = sc.GetStream();
			}
			else
			{
				dataStream = Session_ME.ConnectViaHttpProxyPub(ProxyHost, ProxyPort, host, port, ProxyUsername, ProxyPassword, false, out sc);
			}
		}
		else
		{
